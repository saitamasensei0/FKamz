...
def parse(path) :
    ...
    for dir in dirs : 
        ...
        for file in os.listdir(path + dir + '/') :
            ...
            for i in df.index : 
                last = i
            last = int(last/condition)
            print(last+1)
            for i in df.index :
                if df['Ad'][i] == 1 and (last-2)*condition<=i<(last-1)*condition :
                    ads_serp[0] += 1
                    if df['Pvt'][i] == 1 :
                        only_ad_serp[0] += 1
                if df['Ad'][i] == 1 and (last-2)*condition<=i<(last)*condition :
                    ads_serp[1] += 1
                    if df['Pvt'][i] == 1 :
                        only_ad_serp[1] += 1
                if df['Ad'][i] == 1 and (last-2)*condition<=i<(last+1)*condition :
                    ...
                    if df['Pvt'][i] == 1 :
                        only_ad_serp[2] += 1
                if df['Ad'][i] == 0 and df['Pvt'][i] == 1 and (last-2)*condition<=i<(last-1)*condition:
                    organic_serp[0] += 1
                if df['Ad'][i] == 0 and df['Pvt'][i] == 1 and (last-2)*condition<=i<(last)*condition:
                    organic_serp[1] += 1
                if df['Ad'][i] == 0 and df['Pvt'][i] == 1 and (last-2)*condition<=i<(last+1)*condition:
                    organic_serp[2] += 1
                if df['Pvt'][i] == 1 :
                    ...
                    if i<5 :
                        t5 += 1
                    if (last-2)*condition<=i<(last-1)*condition :
                        serp[0] += 1
                    if (last-2)*condition<=i<(last)*condition :
                        serp[1] += 1
                    if (last-2)*condition<=i<(last+1)*condition :
                        serp[2] += 1
                    if flag :
    ...
    frame.to_csv(path + 'analysis_last_1.csv')
    ...
    frame.to_csv(path + 'analysis_last_2.csv')
    ...
    frame.to_csv(path + 'analysis_last_3.csv')
    ...
    for j in range(0,3) :
        with open(path + 'analysis_last_' + str(j+1) + '.txt','a') as f :
            for i in cat_organic.keys() :
                line = 'In ' + i + ' : ' + '\n'
                f.write(line)
